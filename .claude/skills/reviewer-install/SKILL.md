...
현재 작업 중인 **웹 프로젝트**에 Reviewer 위젯을 설치한다. Reviewer는 페이지 우하단에 떠서 요소에 코멘트를 남기고 Markdown으로 내보내는 리뷰 도구다. 백엔드가 없고(코멘트는 그 사이트 origin의 `localStorage`에만 저장), `?review=1` 링크를 한 번 거친 브라우저에만 보여 **일반 사용자에겐 노출되지 않는다.** 브라우저 북마크는 건드리지 않는다.
...
4. **요약 출력** — 사용자에게 다음을 알린다:
   ...
   - **리뷰어 초대 링크**: `<배포 또는 로컬 URL>/?review=1`
     이 링크를 한 번 연 브라우저에만 위젯이 뜬다(일반 사용자에겐 안 보임). 끄려면 위젯 패널의 **위젯 닫기**.
   - **공용 비밀번호**: 최초 진입 시 이름과 공용 비밀번호를 입력해야 활성화된다(값은 `widget/app.tsx`의 `ACCESS_PASSWORD`). 리뷰어에게 비밀번호를 따로 공유할 것.
