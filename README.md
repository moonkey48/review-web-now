...
### ① 리뷰어 초대
리뷰할 사람에게 주소 끝에 `?review=1`만 붙여 보내면 됩니다:
```
https://내서비스.com/?review=1
```
이 링크를 **한 번 열면** 그 브라우저에선 이후 그 사이트에 💬 버튼이 떠 있습니다. (URL의 `?review`는 자동으로 사라짐)
...
위젯을 고치면 `pnpm build` 후 `dist/widget.js`를 커밋·푸시하세요. CDN(`@main`)은 약 12시간 캐시되니, 즉시 반영하려면 설치 URL을 `@<커밋해시>`로 바꾸면 됩니다.
